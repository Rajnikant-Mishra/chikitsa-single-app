...
export default function RentalForm({
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {/* Dynamic Referral Dropdown from Reference API */}
                <select
                  value={formData?.care_referal || ""}
                  onChange={(e) =>
                    setFormData({ ...formData, care_referal: e.target.value })
  ...
  <button
    ...
    onClick={onCancel}
    disabled={isSubmitting}
    className="w-full sm:w-auto px-6 py-3 border border-slate-200 bg-white hover:bg-slate-50 rounded-xl text-slate-700 font-bold text-sm sm:text-base transition disabled:opacity-50"
  >
    Discard
  </button>
  ...
  <button
    type="submit"
    disabled={isSubmitting}
    className="w-full sm:w-auto px-8 py-3 bg-[#0e4a67] hover:bg-[#125c80] text-white rounded-xl font-extrabold text-sm sm:text-base transition disabled:opacity-50"
  >
    {isSubmitting
      ? "Processing Sequence..."
      : "Save Requisition & Deploy"}
